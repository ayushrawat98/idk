...
class DB {
	...
	constructor() {
		...
		this.queries = {
			...
			// getThreads : this.db.prepare("select t.id, t.title, t.content, t.username, t.created_at, f.path as image_path, f.mime_type as mimetype, count(p.id) as reply_count, (select json_group_array(json_object('id', r.id, 'content', r.content, 'username', r.username, 'created_at', r.created_at)) from (select id, content, username, created_at from posts where parent_id = t.id order by created_at desc limit 3) r) as latest_replies from posts t left join files f on t.file_id = f.id left join posts p on p.parent_id = t.id where t.board_id = ? and t.parent_id is null group by t.id order by t.updated_at desc limit 30;"),
